...
**3.6~13.1cm로 흩어져** 있었다. 호모그래피는 평면↔평면 대응이라 캘리브 점이
전부 같은 높이여야 한다. 카메라 모델로 계산한 결과:

| 카메라 | 보고되는 재투영 오차 | 실제 테이블면 오차 | 수정 후 실제 오차 |
|---|---|---|---|
| 수직 | 3.8mm ("좋음") | 평균 14.7 / 최대 31mm | **0.52mm** |
| 15도 기울임 | 4.9mm ("좋음") | 평균 78.5 / 최대 187mm | **1.81mm** |
| 30도 기울임 | 10.9mm ("사용 가능") | 평균 471mm (파탄) | **3.48mm** |

캘리브 점을 전부 z=0에 두면 오차가 정확히 0이 되어 원인이 높이 불일치임이
특정된다. 수정은 **캘리브 지점을 테이블면 위 (x, y)로 적어두고 관절각을 IK로
푸는 것**이다(`kinematics.TOPDOWN_CALIB_XY`). 높이 폭이 0.00mm가 된다.
