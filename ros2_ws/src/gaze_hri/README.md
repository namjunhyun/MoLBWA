...
**현장에서 필요한 준비도 이 창에서 합니다.**

| 키 | 하는 일 |
|---|---|
| **`k`** | **호모그래피 캘리브레이션** — 팔이 8개 자세를 잡으면 그리퍼 끝을 클릭. `u` 로 직전 취소 |
| **우클릭** | **컵 색 지정** — 컵을 한 번 누르면 그 색으로 검출 범위를 잡고 바로 켭니다 |
| 좌클릭 | 선택 |
| `c` / `space` | 취소 / 비상정지 |
| `d` / `q` | 검출 전환 / 종료 |

캘리브레이션 전에는 화면 가운데에 크게 알려줍니다(그 상태에선 좌표가 안 나옵니다).
카메라 인덱스는 지정한 게 안 열리면 자동으로 다른 인덱스를 찾습니다.
